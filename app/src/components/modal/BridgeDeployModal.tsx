...
import { TransactionAction, TransactionChain, TransactionStatus } from '@/types/bridge.types';
...
export function BridgeDeployModal({
  isOpen,
  onClose,
  bridgeAddress,
  token,
  currentPrice,
}: BridgeDeployModalProps) {
  ...
  const [deploymentEstimates, setDeploymentEstimates] = useState<
    Partial<Record<TransactionChain, DeploymentEstimateStats>>
  >({});
  ...
  const [deploymentOptionsWithStatus, setDeploymentOptionsWithStatus] =
    useState<DeploymentOption[]>(deploymentOptions);
  ...
  const getEstimateText = useCallback(
    (chain?: TransactionChain) => {
      if (!chain) {
        return '—';
      }
      ...
      if (stats) {
        return (
          <div className="text-right space-y-0.5">
            <div className="text-xs font-medium text-gray-700">{getEstimateText(chain)}</div>
  ...
  const checkBalance = (chain: 'sol' | 'near' | 'eth', balance: number, minRequired: number) => {
  ...
  const handleDeploy = async () => {
    ...
    try {
      ...
      const txDeployToken = await deployToken(
        network,
        ChainKind.Sol,
        ChainKind.Near,
        token?.mintAddress,
  ...
  const handleViewOnExplorer = () => {
    if (selectedOption?.name == 'NEAR') {
      window.open(
        `https://${NEAR_NETWORK == 'testnet' && 'testnet.'}nearblocks.io/address/${bridgeAddress[-1]}`,
        '_blank',
      );
    }
  };
          ...
          chainInfo = {
            ...
            explorerUrl:
              NEAR_NETWORK == 'testnet'
                ? `https://testnet.nearblocks.io/address/${signedAccountId!}`
                : `https://nearblocks.io/address/${signedAccountId!}`,
          };
  ...
  const solanaChain = token?.mintAddress
    ? {
        ...
        explorerUrl:
          SOL_NETWORK == 'devnet'
            ? `https://solscan.io/account/${solanaPublicKey!}?cluster=devnet`
            : `https://solscan.io/account/${solanaPublicKey!}`,
      }
    : null;

  const chains: Chain[] = [...(solanaChain ? [solanaChain] : []), ...bridgedChains];
    ...
    <>
      ...
      <Dialog open={shouldShowMainModal} onOpenChange={onClose}>
        <DialogContent className="md:max-w-[500px] max-w-[360px] rounded-lg max-h-[95vh] overflow-y-hidde [&>button]:hidden border-none">
          <DialogHeader className="flex flex-row items-center justify-between">
            <DialogTitle className="text-xl font-semibold">Deploy Bridge Contract</DialogTitle>
          </DialogHeader>

          <Tabs
            value={activeTab}
            onValueChange={(value) => setActiveTab(value as 'bridge' | 'create')}
            className="mt-4"
          >
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      <p className="text-xs text-yellow-800">
                        You need to create a bridge contract on a new chain before you can bridge
                        tokens.
                      </p>
                    </div>
                  </div>
      ...
      <Dialog open={showReviewModal} onOpenChange={handleCancel}>
        <DialogContent className="md:max-w-[500px] max-w-[360px] rounded-lg [&>button]:hidden border-none">
          <DialogHeader>
            ...
            <p className="text-sm text-gray-600 mt-2">Review the details before proceeding</p>
          </DialogHeader>
          ...
          <Card className="mt-1 space-y-5 shadow-none p-2 px-3">
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="flex items-center gap-2">
                <img src={selectedOption?.logo} alt={selectedOption?.name} className="w-5 h-5" />
          ...
          <div className="mt-2 p-2 bg-blue-50 border border-blue-200 rounded-lg">
            <p className="text-xs text-blue-800">
              <strong>Note:</strong> Deployment time varies based on network conditions. Actual time
              will be displayed during deployment.
            </p>
          </div>
      ...
      <Dialog open={showConfirmModal} onOpenChange={handleCancel}>
        <DialogContent className="md:max-w-[500px] max-w-[360px] rounded-lg [&>button]:hidden border-none">
          ...
          <div className="mt-2 p-2 bg-blue-50 border border-blue-200 rounded-lg">
            <p className="text-xs text-blue-800">
              <strong>Note:</strong> Deployment time varies based on network conditions. You'll see
              real-time progress during deployment.
            </p>
          </div>
      ...
      <Dialog open={showProcessingModal} onOpenChange={() => {}}>
        <DialogContent className="md:max-w-[500px] max-w-[360px] rounded-lg [&>button]:hidden border-none">
          <DialogHeader>
            ...
            <p className="text-sm text-gray-600 mt-2">Track deployment progress</p>
          </DialogHeader>
          ...
          <div className="mt-6 text-center">
            ...
            <h3 className="text-lg font-semibold text-gray-900 mb-2">Deploying {token?.symbol}</h3>
            ...
            <p className="text-sm text-gray-600 mb-4">{deploymentProgress}% complete</p>
                ...
                <span className="text-xs font-semibold text-gray-700">
                  {deploymentProgress > 0 && deploymentProgress < 100
                    ...
                    : selectedEstimateMs
                      ...
                      : selectedOption
                        ? getEstimateText(selectedOption.chain)
                        : 'Calculating...'}
                </span>
              </div>
            </div>
            ...
            <p className="text-sm text-gray-500 mt-2">
              ...
              {selectedEstimateMs &&
                selectedEstimateMs > 0 &&
                `Current estimate: ~${formatDuration(selectedEstimateMs)}.`}
            </p>
          </div>
        </DialogContent>
      </Dialog>
          ...
          <div className="text-center space-y-4 mt-2">
            ...
            <div className="text-left flex justify-between items-center mb-6">
              ...
              <div className="flex items-center justify-between gap-2">
                ...
                <div className="flex">
                  <button onClick={handleCopyAddress} className="p-1 hover:bg-gray-200 rounded">
                    <svg
                  ...
                  <button onClick={handleViewOnExplorer} className="p-1 hover:bg-gray-200 rounded">
                    <svg
                      className="w-4 h-4 text-gray-600"
                      fill="none"
                      stroke="currentColor"
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-gray-700">
                    Users can now bridge tokens between {token.symbol} and {selectedOption?.name}
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  <BadgeCheck className="w-5 h-5 text-gray-600" />
      ...
      <Dialog open={showBridgeProcessingModal} onOpenChange={() => {}}>
        <DialogContent className="md:max-w-[500px] max-w-[360px] rounded-lg [&>button]:hidden border-none">
          <DialogHeader>
            ...
            <p className="text-sm text-gray-600 mt-2">Transferring your tokens across chains</p>
          </DialogHeader>
          ...
          <div className="mt-6 text-center">
            ...
            <p className="text-sm text-gray-600 mb-4">{bridgeProgress}% complete</p>
      ...
      <Dialog open={showBridgeSuccessModal} onOpenChange={handleBridgeSuccessClose}>
        <DialogContent className="md:max-w-[500px] max-w-[360px] rounded-lg [&>button]:hidden border-none">
          <DialogHeader>
            ...
            <p className="text-sm text-gray-600 mt-2">Transfer completed successfully</p>
          </DialogHeader>
          ...
          <div className="text-center space-y-4 mt-2">
            <div className="space-y-1 border-b border-gray-200 pb-4">
              ...
              <h3 className="text-lg font-medium text-gray-900">Bridge Successful!</h3>
              <p className="text-sm font-extralight text-gray-600">
                {formatNumberWithCommas(bridgeAmount)} {token?.symbol} has been bridged from{' '}
                {bridgeFromChain} to {bridgeToChain}
              </p>
            </div>
                ...
                <div className="flex items-center justify-between gap-2">
                  ...
                  <div className="flex">
                    ...
                    <button
                      onClick={() =>
                        window.open(
                          `https://solscan.io/tx/${bridgeTransactionHash}?cluster=devnet`,
                          '_blank',
                        )
                      }
                      className="p-1 hover:bg-gray-200 rounded"
                ...
                <div className="flex items-center justify-between gap-2">
                  ...
                  <div className="flex">
                    ...
                    <button
                      onClick={() =>
                        window.open(
                          `https://testnet.nearblocks.io/en/txns/${bridgeTransactionHashNear}`,
                          '_blank',
                        )
                      }
                      className="p-1 hover:bg-gray-200 rounded"
