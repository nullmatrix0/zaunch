...
export function InputBridge({
  ...
  const fetchBalanceToken = useCallback(async () => {
    switch (selectedChain.name) {
      case 'NEAR': {
        const balanceNear = await getTokenBalanceOnNEAR(
      ...
      case 'SOLANA': {
        const balanceSolana = await getTokenBalanceOnSOL(
          selectedChain.address,
          selectedChain.userAddress,
        );
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-1.5">
            <div className="flex items-center gap-1 relative">
              <img
                src={getIpfsUrl(token?.metadata?.tokenUri)}
                alt={token?.symbol}
              ...
              <img
                ...
                className="w-3 h-3 rounded-full absolute -bottom-0.5 right-0"
              />
            </div>
          ...
          <a
            ...
            target="_blank"
            className="flex items-center gap-2 text-xs text-gray-500 cursor-pointer hover:underline"
          >
        ...
        <div className="flex items-center justify-between w-full">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                variant="outline"
                className="flex items-center gap-2 px-2 py-2 bg-gray-50 h-auto border border-gray-200 shadow-none hover:bg-gray-100 hover:text-gray-600 cursor-pointer"
              >
                {selectedChain ? (
                  <>
                    <img
