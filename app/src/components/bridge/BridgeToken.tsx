...
import { AlertCircle, ArrowUpDown, Check } from 'lucide-react';
...
export default function BridgeToken() {
  ...
  const getWalletAddress = useCallback(
    (chain: ChainType): string | undefined => {
      switch (chain) {
        case 'solana':
  ...
  const fetchBridgeTokens = useCallback(async () => {
    if (selectedToken) {
      ...
      const addressTokenBridged = await getAllBridgeTokens(
        selectedToken.mint,
        chainToken,
        'testnet',
      );
  ...
  const checkBalance = useCallback(
    (chain: 'sol' | 'near' | 'eth', balance: number, minRequired: number) => {
  ...
  const handleBridge = useCallback(async () => {
    ...
    try {
      ...
      const transactionPayload = {
        ...
        chain:
          fromChain === 'solana'
            ? TransactionChain.SOLANA
            : fromChain === 'near'
              ? TransactionChain.NEAR
              : TransactionChain.ETHEREUM,
      ...
      const amountBigInt = BigInt(parseFormattedNumber(amount) * 10 ** selectedToken.decimals);
  ...
  const handleToChainChange = useCallback(
    (chain: ChainType) => {
      if (chain === fromChain) {
        toast.error(
          'Bridge only works for cross-chain transfers. Please select a different destination chain.',
        );
        return;
      }
      setToChain(chain);
  ...
  const handleFromChainChange = useCallback(
    (chain: ChainType) => {
      if (chain === toChain) {
        const availableChains: ChainType[] = ['solana', 'near', 'ethereum'].filter(
          (c) => c !== chain,
        ) as ChainType[];
  ...
  return (
    <div className="min-h-screen py-4 md:py-8">
              ...
              <div className="flex flex-col">
                ...
                <div className="flex items-start gap-2 mt-4 p-3 bg-orange-50 border border-orange-200 rounded-lg">
                  ...
                  <p className="text-xs text-orange-700">
                    Bridge only supports cross-chain transfers. Token swaps on the same chain are
                    not available.
                  </p>
                </div>
      ...
      <Dialog open={modalState.showBridgeProcessing} onOpenChange={() => {}}>
        <DialogContent className="w-[90%] max-w-[500px] rounded-lg [&>button]:hidden border-none">
          ...
          <div className="mt-6 text-center">
            <div className="flex justify-center mb-4">
              <div className="flex items-center gap-4">
                <img
                  src={
                    fromChain === 'solana'
                      ? '/chains/solana-dark.svg'
                      : fromChain === 'near'
                        ? '/chains/near-dark.svg'
                        : '/chains/ethereum.svg'
                  }
                  alt={fromChain}
                  className="h-12 w-12"
            ...
            <p className="text-sm text-gray-600 mb-4">{modalState.bridgeProgress}% complete</p>
      ...
      <Dialog
        open={modalState.showBridgeSuccess}
        onOpenChange={() => dispatchModal({ type: 'CLOSE_BRIDGE_SUCCESS' })}
      >
        <DialogContent className="w-[90%] max-w-[500px] rounded-lg [&>button]:hidden border-none">
          <DialogHeader>
            <DialogTitle className="text-lg font-semibold">
          ...
          <div className="text-center space-y-4 mt-2">
            <div className="space-y-1 border-b border-gray-200 pb-4">
              ...
              <p className="text-sm font-extralight text-gray-600">
                {amount} {selectedToken?.symbol} has been bridged from {fromChain.toUpperCase()} to{' '}
                {toChain.toUpperCase()}
              </p>
            </div>
          </div>
      ...
      <Dialog open={modalState.showDeployProcessing} onOpenChange={() => {}}>
        <DialogContent className="w-[90%] max-w-[500px] rounded-lg [&>button]:hidden border-none">
          <DialogHeader>
            ...
            <p className="text-sm text-gray-600 mt-2">Creating your token on the target chain</p>
          </DialogHeader>
          ...
          <div className="mt-6 text-center">
            ...
            <p className="text-sm text-gray-600 mb-4">{modalState.deployProgress}% complete</p>
      ...
      <Dialog
        open={modalState.showDeploySuccess}
        onOpenChange={() => dispatchModal({ type: 'CLOSE_DEPLOY_SUCCESS' })}
      >
        <DialogContent className="w-[90%] max-w-[500px] rounded-lg [&>button]:hidden border-none">
          <DialogHeader>
            <DialogTitle className="text-lg font-semibold">
            ...
            <p className="text-sm text-gray-600 mt-2">Deployment completed successfully</p>
          </DialogHeader>
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-gray-700">
                    You can now bridge tokens between chains
                  </span>
                </div>
              </div>
            </div>
