...
import { BRIDGE_PROGRAM_ID } from '@/configs/env.config';
...
export function useBridge(): UseBridgeReturn {
  ...
  const checkVault = useCallback(async (tokenMint: string): Promise<VaultStatus> => {
  ...
  const initVault = useCallback(async (tokenMint: string): Promise<string | null> => {
  ...
  const bridge = useCallback(async (params: BridgeParams): Promise<BridgeResult | null> => {
    if (!publicKey || !sendTransaction) {
      toast.error('Please connect your wallet');
      return null;
    }
