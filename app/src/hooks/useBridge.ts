  ...
  const deployToken = async (
    ...
    try {
      ...
      const deployFromNear = async () => {
        ...
        const isValidNearAccount =
          tokenAddress.endsWith('.testnet') ||
          tokenAddress.endsWith('.near') ||
          tokenAddress.length === 64; // implicit account

        if (!isValidNearAccount) {
          throw new Error(
            `Invalid NEAR token address format: ${tokenAddress}. Must end with .testnet, .near, or be a 64-character implicit account.`,
          );
        }
