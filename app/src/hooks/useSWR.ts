...
import { API_URL, getPurchasedTokens } from '@/lib/api';
import { type Token, type Transaction, TransactionAction } from '@/types/api';
...
export function useTransactionBridge(address?: string): {
  ...
  const filteredTransactions = allTransactions.filter(
    (tx: Transaction) =>
      tx.action === TransactionAction.BRIDGE || tx.action === TransactionAction.DEPLOY,
  );
